...
class AVRFlowPostgresGUI(tk.Tk):
    ...
    def run_stage(
        ...
        try:
            ...
            self.status_var.set(
                f"{self.stage_state[key]['label']} passed — "
    ...
    def _build_consumers_tab(self):
        ...
        ttk.Button(
            ...
            style="Accent.TButton",
            command=self.run_all_consumers,
        ).pack(side="right")
    ...
    def refresh_reconciliation(self):
        ...
        batch_amount_rows = int(
            batch_amount_rows or 0
        )

        stream_amount_rows = int(
            stream_amount_rows or 0
        )
        ...
        reconciliation_rows = [
            ...
            (
                "Rows with valid Actual Amount Paid",
                fmt_number(batch_amount_rows),
                fmt_number(stream_amount_rows),
                fmt_number(
                    abs(
                        batch_amount_rows
                        - stream_amount_rows
        ...
        conditions = [
            (
                (
                    "Ordered stream record count equals "
                    "direct database record count"
                ),
                "EXACT",
                (
                    "yes"
                    if batch_total == stream_total
                    else "no"
                ),
            ),
            (
                (
                    "Ordered stream Actual Amount Paid row "
                    "count equals direct database aggregate"
                ),
                "EXACT",
                (
                    "yes"
            ...
            (
                (
                    "Ordered stream Actual Recorded Revenue "
                    "equals direct database aggregate"
                ),
                "EXACT",
                (
                    "yes"
            ...
            (
                (
                    "Calendar listing_id values resolve "
                    "to listings.id"
                ),
                "EXACT",
                (
            ...
            (
                (
                    "Review listing_id values resolve "
                    "to listings.id"
                ),
                "EXACT",
                (
            ...
            (
                (
                    ...
                    "(listing_id, date) has no duplicates"
                ),
                "EXACT",
                (
                    "yes"
        ...
        self.reconciliation_summary_var.set(
            f"{result_word} — Listed Price, Payment Status, "
            f"and Actual Amount Paid were kept separate, "
            f"and Actual Recorded Revenue was compared "
            f"through independent PostgreSQL query paths."
        )
        ...
        self.log_stage(
            ...
            [
                (
                    "Comparison path A: direct PostgreSQL "
                    "aggregate over public.calendar"
                ),
                (
                    "Comparison path B: explicitly ordered "
                    "PostgreSQL stream query"
                ),
                "",
                "REVENUE MODEL",
                (
                    "Listed Price is not treated as "
                    "actual revenue."
                ),
                (
                    "Actual Recorded Revenue is calculated "
                ...
                (
                    ...
                    f"difference="
                    f"{abs(batch_total-stream_total):,}"
                ),
                (
                    "Amount paid rows : "
                    f"batch={batch_amount_rows:,} | "
                    f"stream={stream_amount_rows:,} | "
                    f"difference="
                    f"{abs(batch_amount_rows-stream_amount_rows):,}"
                ),
                (
                    "Actual revenue   : "
                    f"batch={fmt_money(batch_revenue)} | "
                    f"stream={fmt_money(stream_revenue)} | "
                    f"diff={fmt_money(revenue_difference)}"
                ),
                "",
                (
                    "Orphan calendar rows : "
        ...
        return (
            ...
            f"Actual Recorded Revenue "
            f"{fmt_money(batch_revenue)} · "
            f"difference "
            f"{fmt_money(revenue_difference)}"
        )
